...
class S6Block(nn.Module):
    def __init__(self, in_channels, out_channels, latent_dim=512, h0 = None):
        ...
        self.del_proj = nn.Linear(latent_dim*2, latent_dim)
    ...
    def forward(self, x: torch.Tensor):
        ...
        h = torch.zeros_like(A_bar[:, 0, :], dtype=torch.float32, device=x.device)
        batch, L, _ = A_bar.shape
        y = torch.zeros((batch, L, self.latent_dim), dtype=torch.float32, device=x.device)

        # sequential h update O(NL)
        #for t in range(L):
        #    h = A_bar[:, t, :] * h + B_bar[:, t, :] * x_proj[:, t, :]
        #    y[:, t, :] = C[:, t, :] * h

        # parallel scan O(L)
        A_scan = torch.cumprod(A_bar, dim=-2)

        A_rev = torch.flip(A_bar, dims=[1])
        A_rev_prod = torch.flip(torch.cumprod(A_rev, dim=1), dims=[1]) # cumulative product of A from the back

        B_input = B_bar * x_proj
        B_scan = torch.cumsum(A_rev_prod * B_input, dim=1)


        h = (A_scan * self.h0) + B_scan
        y = (C*h) + self.D(x_proj)
